...
REPEの変換(特にエンコード・デコード時の組合せ順位計算)は大量データでは純Pythonがボトルネックになりやすいため、cshogi本体と同様のC++コア+Cythonバインディングによる高速版を `CommonLib/repe_native/` に用意しています。`cd CommonLib/repe_native && python setup.py build_ext --inplace` でビルドすると `convert_teacher.py` は自動的にそちらを使うようになります(未ビルドでも動作します。詳細は `CommonLib/repe_native/README.md` 参照)。
